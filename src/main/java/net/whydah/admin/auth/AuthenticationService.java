...
public class AuthenticationService {
    ...
    @Autowired
    public AuthenticationService(UibAuthConnection uibAuthConnection,
    ...
    public boolean resetPassword(String applicationtokenId,
                                  String username,
                                  UserAction userAction,
                                  String resetPasswordTemplateName) throws AppException {
        ...
        try {
            ...
            if (resetPasswordToken == null || resetPasswordToken.length() < 7) {
                ...
                passwordResetOk = sendNotification(
                        email, cellPhone, username,
                        userAction, resetPasswordToken, resetPasswordTemplateName);
            }
        } catch (IOException e) {
            log.warn("Failed to parse response from uibAuthConnection.resetPassword. " +
                     "username={}, responseJson={}", username, passwordResetJson);
        }
        return passwordResetOk;
    }

    protected boolean sendNotification(String userEmail,
                                        String cellPhone,
                                        String username,
                                        UserAction userAction,
        ...
        if (userAction != null && userAction == UserAction.PIN) {
            // TODO: PIN notification - see
            //   https://github.com/Cantara/Whydah-UserAdminService/issues/31
            throw new UnsupportedOperationException(
                    "PIN-based password reset is not yet implemented. " +
                    "See https://github.com/Cantara/Whydah-UserAdminService/issues/31");
        }

        if (userEmail != null && !userEmail.isEmpty()) {
            notificationIsSent = passwordSender.sendResetPasswordEmail(
                    username, passwordResetToken, userEmail, resetPasswordTemplateName);
        }
